  ...
  <div class="space-y-6">
    ...
    <section v-else class="rounded-[26px] border border-slate-200 bg-white p-4 shadow-[0_18px_40px_rgba(15,23,42,0.08)] sm:p-6">
      ...
      <div v-else class="space-y-6">
        <section
          v-for="grupo in gruposJugadorasActivas"
          :key="grupo.id"
          class="space-y-3"
        >
          <div class="flex items-center justify-between border-b border-slate-200 pb-2">
            ...
            <span class="rounded-full bg-slate-100 px-3 py-1 text-[11px] font-black text-slate-600">
              {{ grupo.jugadoras.length }}
            </span>
          </div>

          <article
            v-for="j in grupo.jugadoras"
            :key="`${grupo.id}-${equipoActivo}-${j.id}`"
          ...
          >
              ...
              >
                ...
                <div class="min-w-0 flex-1">
                  ...
                  <p class="mt-1 text-[11px] font-semibold text-slate-400">
                    Nacimiento: {{ formatearFechaNacimiento(j.fechaNacimiento) }}
                  </p>
                </div>
            ...
            <div v-if="expandidaId === j.id" class="border-t border-slate-100 bg-slate-50/70 px-4 py-4 sm:px-5">
              <div class="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">
                ...
                <div class="rounded-2xl bg-white px-3 py-3 ring-1 ring-slate-200">
                  <p class="text-[10px] font-black uppercase tracking-[0.18em] text-slate-400">Nacimiento</p>
                  <p class="mt-1 text-sm font-semibold text-slate-700">{{ formatearFechaNacimiento(j.fechaNacimiento) }}</p>
                </div>
              </div>
...
const estadosNoDisponibles = ['no_disponible', 'lesionada', 'recuperacion', 'vacaciones'];

const gruposJugadorasActivas = computed(() => {
  const disponibles = [];
  const noDisponibles = [];

  jugadorasFiltradasActivas.value.forEach((jugadora) => {
    if (estadosNoDisponibles.includes(jugadora.estadoSalud)) {
      noDisponibles.push(jugadora);
      return;
    }

    disponibles.push(jugadora);
  });

  return [
    {
      id: 'disponibles',
      titulo: 'Jugadoras disponibles',
      jugadoras: disponibles
    },
    {
      id: 'no-disponibles',
      titulo: 'Jugadoras no disponibles',
      jugadoras: noDisponibles
    }
